...
- `/usr/share/nginx/` — fichiers fournis par le paquet (`mime.types`, `fastcgi_params`…)
- `/var/log/nginx/` — logs
...
# ------------------------------
server {
    server_name domoticz.phlegros.fr;
    ...
    ssl_certificate /etc/letsencrypt/live/phlegros.fr/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/phlegros.fr/privkey.pem; # managed by Certbot
...
# ------------------------------
server {
    server_name ha.phlegros.fr;

    location / {
        proxy_pass http://192.168.0.200:8123;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
...
# ------------------------------
server {
    server_name file.phlegros.fr;

    location / {
        proxy_pass http://192.168.0.200:8888;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        ...
        access_log /var/log/nginx/filebrowser_access.log combined buffer=16k flush=1m;
    }
...
# ------------------------------
server {
    server_name esphome.phlegros.fr;

    location / {
        proxy_pass http://192.168.0.200:6052;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    listen 443 ssl; # managed by Certbot
    ssl_certificate /etc/letsencrypt/live/esphome.phlegros.fr/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/esphome.phlegros.fr/privkey.pem; # managed by Certbot
    include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
    ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
...
# ------------------------------
server {
    server_name phlegros.fr www.phlegros.fr;
...
server {
    ...
    listen 443 ssl;
    ssl_certificate /etc/nginx/ssl/phlegros.fr/phlegros.crt;       # Sur RHEL: /etc/pki/tls/certs/phlegros.crt
    ssl_certificate_key /etc/nginx/ssl/phlegros.fr/phlegros.key;   # Sur RHEL: /etc/pki/tls/private/phlegros.key
    ssl_trusted_certificate /etc/nginx/ssl/phlegros.fr/phlegros.chain; # Sur RHEL: /etc/pki/tls/certs/phlegros.chain

    # Certains fournisseurs livrent un fichier "bundle" contenant la chaîne complète.
    # Dans ce cas, utiliser ssl_trusted_certificate avec ce fichier.
...
- `proxy_set_header Connection "";` — pour éviter les problèmes de WebSocket sur certains services
