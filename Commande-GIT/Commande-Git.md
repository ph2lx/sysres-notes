...
pour se connecter a un git dans un dossier
cd C:\Users\moi\sysres-notes
git pull

# Pour créer un nouveau dossier et l'activer dans Git
...
# Pour mettre à jour tous les fichiers modifiés d'un coup
git add -u
...
1# creation de dossier Linux & gitkeep
New-Item -ItemType Directory -Path Linux
New-Item -ItemType File -Path Linux\.gitkeep
# creation de fichier Nginx
New-Item -ItemType File -Path Linux\NginX

2#
git add Linux\Nginx.md
git add 
git add Linux\.gitkeep
git status
...
1️⃣ (si ce n’est pas déjà fait) – créer les sous‑dossiers GPO
powershell
...
2️⃣ Mettre un fichier dans chaque sous‑dossier (sinon Git les ignore)
powershell
...
3️⃣ Vérifier ce que Git voit
powershell
git status
Tu dois voir les .gitkeep en “untracked” ou “modified”.

4️⃣ Ajouter tout ce qui concerne GPO
powershell
git add Windows-ActiveDirectory-Exploitation/GPO
5️⃣ Commit
powershell
git commit -m "Ajout des sous-dossiers GPO avec .gitkeep"
6️⃣ Push (sur ta branche actuelle)
Si ta branche est toujours Windows-ActiveDirectory-Exploitation/Comptes_Acces :

powershell
git push
(si Git râle qu’il n’y a pas d’upstream, alors une fois pour toutes:)
...
git commit -m "Ajout du dossiers Comptes_et_Acces avec .gitkeep"
git push

pour deplacer un txt dans git :
soit mv soit en gui

puis se placer à la racine du dossier et :
git add .
